<template>
  ...
  <section
    ...
    :class="
      isFriendSelected() || (hasSelectedUser && isAdmin)
        ? 'justify-center'
        : 'justify-center'
    "
  >
    <Button
      v-if="isAdmin"
      :label="$t('buttons.addGuest')"
      @onClicked="onCreateFriend"
      class="separator"
    />
    <div
      v-if="isFriendSelected() || hasSelectedUser"
      class="join-group separator"
      @click="onGoMyFriend"
      :data-text="$t('buttons.myFriend')"
    >
      {{ $t("buttons.myFriend") }}
    </div>
...
const { groups, group, groupsUser, setCurrentGroup, setGroupsUser } =
  useGroups();
const {
  guests,
  isLoading,
...
const getUsers = computed(() =>
  usersSearch.value.filter((user) =>
    user.user.toLowerCase().includes(dataFriend.name.toLowerCase())
  )
);
...
const onSubmitFriend = async () => {
  const isExistGuest = guests.value.guests.some(
    (guest) => guest.id === idGuest.value
  );
  if (isExistGuest) {
