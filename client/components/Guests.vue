<template>
  <Transition name="slide-fade">
    ...
    <div v-else class="w-full">
      ...
      <div
        id="guests"
        class="w-full flex justify-center items-center flex-wrap mt-5"
      >
              ...
              <template v-if="isAdmin">
                <div @click="onOpenModalDelete(guest.hashGuest, params)">
                  <font-awesome-icon
                    ...
                    class="icon text-3xl cursor-pointer"
                  />
                </div>
                <div @click="onSelectedFriend(guest)">
                  <font-awesome-icon
                    icon="fa-solid fa-hand-pointer"
                    class="icon text-3xl cursor-pointer ml-3"
                    title="select user"
                  />
                </div>
              </template>
        ...
        <template v-for="guest in selectedFriends" :key="guest.id">
          ...
          >
            ...
            <div class="icon-container" v-if="selectedGuestId === guest.id">
              <div
                v-if="isAdmin"
                @click="onOpenModalDelete(guest.hashGuest, params)"
              >
                <font-awesome-icon
                  icon="fa-solid fa-trash"
                  class="icon text-3xl cursor-pointer"
      ...
      <Modal :show="isDeleteGuest" @onClose="onCloseModalDelete" padding>
        <div class="text-center">
          ...
          <div class="flex justify-around">
            <Button
              :label="$t('modals.deleteGuest.accept')"
              @onClicked="deleteGuest()"
            />
            <Button
              :label="$t('modals.deleteGuest.cancel')"
              @onClicked="onCloseModalDelete"
            />
          </div>
        </div>
      </Modal>
...
const unselectedFriends = computed(() =>
  guests.value?.guests.filter((guest) => guest.active === 0)
);
...
const onSelectedFriend = async (guest) => {
  ...
  if (guest.id === authUser.value.id) {
    ...
    const updatedGuest = {
      friend: guest.id,
      active: 1,
      idFriend: guests.value.guests.find((g) => g.id === authUser.value.id)
        .hashGuest,
